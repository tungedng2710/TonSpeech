...
class STFT(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if version.parse(torch.__version__) <= version.parse("1.6.0"):
            stft_extractor = tl.STFT(n_fft=self.n_fft, 
...
class ISTFT(torch.nn.Module):
    ...
    def forward(self, x, sig_length=None):
        ...
        # )
        # print(istft1.shape)
        real = x[:,:,:,0:1].movedim(3,0).movedim(3,2)
        imag = x[:,:,:,1:2].movedim(3,0).movedim(3,2)
        torch.save(real, "real.pt")
        torch.save(imag, "imag.pt")
        ...
        #                             onnx=True)
        # sig_length = torch.Tensor([sig_length]).type(torch.LongTensor)
